...
def CalculateTauIDWeight(self,theTree):
    ...
    self.value[0] = self.SFTool.getSFvsPT(tauVector.Pt(),genmatch=theTree.gen_match_2)

def CalculateTauIDWeight_pT0to35_UP(self,theTree,uncert):
    tauVector = ROOT.TLorentzVector()
    tauVector.SetPtEtaPhiM(theTree.pt_2,theTree.eta_2,theTree.phi_2,theTree.m_2)
    self.uncertaintyVariationArrays[uncert][0] = self.SFTool.getSFvsPT(tauVector.Pt(),genmatch=theTree.gen_match_2)
    if(tauVector.Pt() < 35.0):
        self.uncertaintyVariationArrays[uncert][0] = self.SFTool.getSFvsPT(tauVector.Pt(),genmatch=theTree.gen_match_2,unc='Up')
...
def CalculateTauIDWeight_pT0to35_DOWN(self,theTree,uncert):
    ...
    self.uncertaintyVariationArrays[uncert][0] = self.SFTool.getSFvsPT(tauVector.Pt(),genmatch=theTree.gen_match_2)
    if(tauVector.Pt() < 35.0):
        self.uncertaintyVariationArrays[uncert][0] = self.SFTool.getSFvsPT(tauVector.Pt(),genmatch=theTree.gen_match_2,unc='Down')
